...
namespace stormkit::gpu {
    ...
    Buffer::Buffer(const Device& device, const CreateInfo& info, bool persistently_mapped)
        ...
        try {
            m_vk_buffer = vk::raii::Buffer { this->device().vkHandle(), create_info };
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto Buffer::operator=(Buffer&& other) noexcept -> Buffer& = default;
        ...
        for (auto i : core::range(mem_properties.memoryTypeCount)) {
            if ((type_filter & (1 << i)) and
                (checkFlag(mem_properties.memoryTypes[i].propertyFlags,
                           core::as<vk::MemoryPropertyFlagBits>(properties.m_mask))))
                return i;
        }
