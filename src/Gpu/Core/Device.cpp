...
namespace stormkit::gpu {
    ...
    Device::Device(const PhysicalDevice& physical_device, const Instance& instance, Info info)
        ...
        m_vk_device =
            std::make_unique<vk::raii::Device>(m_physical_device->vkHandle(), create_info);
        ...
        const auto alloc_create_info = vma::AllocatorCreateInfo {}
                                           .setInstance(*(this->instance().vkHandle()))
                                           .setPhysicalDevice(*physical_device.vkHandle())
                                           .setDevice(*vkHandle())
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto Device::operator=(Device&& other) noexcept -> Device& = default;
    ...
    auto Device::waitForFences(std::span<const core::NakedRef<const Fence>> fences,
                               bool wait_all,
                               const std::chrono::milliseconds& timeout) const noexcept -> Result {
        core::expects(m_vk_device != nullptr);

        auto vk_fences = fences |
                         std::views::transform([](auto&& fence) { return *fence->vkHandle(); }) |
                         std::ranges::to<std::vector>();

        return core::as<Result>(vkHandle().waitForFences(
            vk_fences,
            wait_all,
    ...
    auto Device::resetFences(std::span<const core::NakedRef<const Fence>> fences) const noexcept
        ...
        try {
            vkHandle().resetFences(vk_fences);
        } catch (const vk::SystemError& err) { return core::as<Result>(err.code().value()); }
    ...
    auto Device::setObjectName(core::UInt64 object,
        ...
        vkHandle().setDebugUtilsObjectNameEXT(info);
    }
} // namespace stormkit::gpu
