...
// Configuration file with compile-time options (edit imconfig.h or '#define IMGUI_USER_CONFIG
// "myfilename.h" from your build system')
    ...
    // files (imgui_impl_xxx.h) Using dear imgui via a shared library is not recommended, because we
    // don't guarantee backward nor forward ABI compatibility (also function call overhead, as dear
    // imgui is a call-heavy API)
    #ifndef IMGUI_API
        #define IMGUI_API
    #endif
    ...
    #define IM_UNUSED(_VAR) \
        ...
        #define IM_OFFSETOF(_TYPE, _MEMBER) \
            offsetof(_TYPE, _MEMBER) // Offset of _MEMBER within _TYPE. Standardized as offsetof()
                                     // in C++11
    #else
        #define IM_OFFSETOF(_TYPE, _MEMBER) \
...
typedef int ImGuiTableBgTarget; // -> enum ImGuiTableBgTarget_   // Enum: A color target for
                                // TableSetBgColor()
typedef int ImDrawCornerFlags;  // -> enum ImDrawCornerFlags_    // Flags: for ImDrawList functions:
                                // AddRect(), AddRectFilled() etc.
typedef int ImDrawFlags;        // -> enum ImDrawFlags_          // Flags: for ImDrawList functions:
                                // AddPolyline(), PathStroke() etc.
...
namespace ImGui {
    ...
    IMGUI_API void
        ShowFontSelector(const char *label); // add font selector block (not a window), essentially
    ...
    IMGUI_API ImVec2
        GetWindowContentRegionMax(); // content boundaries max (roughly (0,0)+Size-Scroll) where
                                     // Size can be override with SetNextWindowContentSize(), in
                                     // window coordinates
    ...
    IMGUI_API void PushButtonRepeat(
        bool repeat); // in 'repeat' mode, Button*() functions return repeated true in a typematic
                      ...
                      // can call IsItemActive() after any Button() to tell if the button is held in
                      // the current frame.
    ...
    IMGUI_API ImU32
        GetColorU32(const ImVec4 &col); // retrieve given color with style alpha applied, packed as
    ...
    IMGUI_API float GetCursorPosY(); //    other functions such as GetCursorScreenPos or everything
                                     //    in ImDrawList::
    IMGUI_API void
    ...
    IMGUI_API void
        AlignTextToFramePadding(); // vertically align upcoming text baseline to FramePadding.y so
                                   // that it will align properly to regularly framed items (call if
                                   // you have text on a line before a framed item)
    ...
    IMGUI_API bool ColorButton(const char *desc_id,
                               ...
                               ImVec2 size               = ImVec2(0,
                                                    0)); // display a color square/button, hover for
    ...
    IMGUI_API void
        OpenPopupOnItemClick(const char *str_id = NULL,
                             ImGuiPopupFlags popup_flags =
                                 1); // helper to open popup when clicked on last item. return true
    ...
    //  add other flags remember to re-add the ImGuiPopupFlags_MouseButtonRight.
    IMGUI_API bool BeginPopupContextItem(
        const char *str_id = NULL,
        ImGuiPopupFlags popup_flags =
            1); // open+begin popup when clicked on last item. if you can pass a NULL str_id only if
                // the previous item had an id. If you want to use that on a non-interactive item
    ...
    // Tables
    // [BETA API] API may evolve slightly! If you use this, please update to the next version when
    // it comes out!
    ...
    // Tables: Sorting
    // - Call TableGetSortSpecs() to retrieve latest sort specs for the table. NULL when not
    // sorting.
    // - When 'SpecsDirty == true' you should sort your data. It will be true when sorting specs
    // have changed
    ...
    IMGUI_API void
        TableSetBgColor(ImGuiTableBgTarget target,
                        ImU32 color,
    ...
    IMGUI_API bool
        IsItemDeactivatedAfterEdit(); // was the last item just made inactive and made a value
                                      // change when it was active? (e.g. Slider/Drag moved). Useful
                                      // for Undo/Redo patterns with widgets that requires
    ...
    IMGUI_API ImDrawList *
        GetBackgroundDrawList(); // this draw list will be the first rendering one. Useful to
                                 // quickly draw shapes/text behind dear imgui contents.
    IMGUI_API ImDrawList *
        GetForegroundDrawList(); // this draw list will be the last rendered one. Useful to quickly
    ...
    // - We don't know the meaning of those value. You can use GetKeyIndex() to map a ImGuiKey_
    ...
    IMGUI_API ImVec2
        GetMousePosOnOpeningCurrentPopup(); // retrieve mouse position at the time of opening popup
                                            // we have BeginPopup() into (helper to avoid user
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_PadOuterX = 1 << 21, // Default if BordersOuterV is on. Enable outer-most
...
// Obsolete names (will be removed soon)
    ...
    // ImGuiTableFlags_ColumnsWidthStretch = ImGuiTableFlags_SizingStretchSame   // WIP Tables
    // 2020/12 , ImGuiTableFlags_SizingPolicyFixed = ImGuiTableFlags_SizingFixedFit,
    // ImGuiTableFlags_SizingPolicyStretch = ImGuiTableFlags_SizingStretchSame   // WIP Tables
    // 2021/01
    #endif
};
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_NoClip = 1 << 7, // Disable clipping for this column (all NoClip columns
                                           // will render in a same draw command).
    ImGuiTableColumnFlags_NoSort = 1 << 8, // Disable ability to sort on this field (even if
...
// Obsolete names (will be removed soon)
    ...
    // ImGuiTableColumnFlags_NoResize, // Column will not stretch and keep resizing based
    // on submitted contents.
    #endif
};
...
namespace ImGui {
    ...
    } // Bool return value removed. Use IsWindowAppearing() in BeginPopup() instead. Renamed
    ...
    static inline bool
        BeginPopupContextWindow(const char *str_id, ImGuiMouseButton mb, bool over_items) {
...
typedef void (*ImDrawCallback)(const ImDrawList *parent_list, const ImDrawCmd *cmd);
    ...
    // call a function at this address. This is useful for example if you submitted callbacks which
    // you know have altered the render state and you want it to be restored. It is not done by
    // default because they are many perfectly useful way of altering render state for imgui
...
struct ImDrawCmd {
    ...
    // ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset in the renderer
    // backend (recommended). To use 32-bit indices: override with '#define ImDrawIdx unsigned int'
    // in imconfig.h.
    #ifndef ImDrawIdx
...
enum ImDrawFlags_ {
    ImDrawFlags_None = 0,
    ImDrawFlags_Closed =
        1 << 0 // PathStroke(), AddPolyline(): specify that (LEGACY: this must always stay == 1 to
               // be backward compatible with old API using a bool)
};
...
struct ImDrawList {
    ...
    IMGUI_API void PathBezierCubicCurveTo(const ImVec2 &p2,
                                          ...
                                          int num_segments = 0); // Cubic Bezier (4 control points)
    IMGUI_API void
        PathBezierQuadraticCurveTo(const ImVec2 &p2,
                                   const ImVec2 &p3,
                                   int num_segments = 0); // Quadratic Bezier (3 control points)
    ...
    IMGUI_API void
        AddDrawCmd(); // This is useful if you need to forcefully create a new draw call (to allow
...
struct ImDrawData {
    ...
    IMGUI_API void
        DeIndexAllBuffers(); // Helper to convert all buffers from indexed to non-indexed, in case
                             // you cannot render indexed. Note: this is slow and most likely a
...
struct ImFontAtlas {
    ...
    IMGUI_API const ImWchar *
        GetGlyphRangesChineseFull(); // Default + Half-Width + Japanese Hiragana/Katakana + full set
