...
//  by using ImGui_ImplVulkan_LoadFunctions(). 2020-11-11: Vulkan: Added support for specifying
//  which subpass to reference during VkPipeline creation. 2020-09-07: Vulkan: Added VkPipeline
//  parameter to ImGui_ImplVulkan_RenderDrawData (default to one passed to ImGui_ImplVulkan_Init).
//  2020-05-04: Vulkan: Fixed crash if initial frame has no vertices.
//  2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be called if the ImDrawData
//  didn't have vertices. 2019-08-01: Vulkan: Added support for specifying multisample count. Set
//  ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to use,
//  default is non-multisampled as before. 2019-05-29: Vulkan: Added support for large mesh (64K+
//  vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Vulkan: Added support
//  for special ImDrawCallback_ResetRenderState callback to reset render state. 2019-04-04:
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance,
    ...
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if "
                                   "IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    (void)instance;
    ImGui_ImplVulkanH_CreateWindowSwapChain(physical_device,
                                            device,
                                            wd,
                                            allocator,
                                            width,
                                            height,
                                            min_image_count);
    ImGui_ImplVulkanH_CreateWindowCommandBuffers(physical_device,
                                                 device,
                                                 wd,
                                                 queue_family,
                                                 allocator);
...
void ImGui_ImplVulkanH_DestroyWindowRenderBuffers(VkDevice device,
    ...
    for (uint32_t n = 0; n < buffers->Count; n++)
        ImGui_ImplVulkanH_DestroyFrameRenderBuffers(device,
