...
namespace ImStb {
    ...
    // Error handling
    // Down the line in some frameworks/languages we would like to have a way to redirect those to
    // the programmer and recover from more faults.
...
IMGUI_API int ImTextCountUtf8BytesFromStr(
    ...
    // Helpers: ImVec2/ImVec4 operators
    // We are keeping those disabled by default so they don't leak in user space, to allow user
    // enabling implicit cast operators between ImVec2 and their own types (using
    // IM_VEC2_CLASS_EXTRA etc.) We unfortunately don't have a unary- operator for ImVec2 because
    // this would needs to be defined inside the class itself.
...
IMGUI_API void *ImFileLoadToMemory(const char *filename,
    ...
    // the code easy to copy)
        ...
        #define ImFloorStd(X) \
            floorf(           \
                X) // We already uses our own ImFloor() { return (float)(int)v } internally so the
...
struct ImGuiViewportP: public ImGuiViewport {
    ...
    void UpdateWorkRect() {
        ...
        WorkSize = ImVec2(ImMax(0.0f, Size.x - WorkOffsetMin.x + WorkOffsetMax.x),
...
// FIXME-TABLE: transient data could be stored in a per-stacked table structure: DrawSplitter,
...
namespace ImGui {
    ...
    IMGUI_API void Shutdown(ImGuiContext *context); // Since 1.60 this is a _private_ function. You
    ...
    IMGUI_API void SetLastItemData(ImGuiWindow *window,
                                   ImGuiID item_id,
    ...
    IMGUI_API void
        SetNavID(ImGuiID id, int nav_layer, ImGuiID focus_scope_id, const ImRect &rect_rel);
    ...
    IMGUI_API int
        TableGetHoveredColumn(); // May use (TableGetColumnFlags() &
                                 // ImGuiTableColumnFlags_IsHovered) instead. Return hovered column.
                                 // return -1 when table is not hovered. return columns_count if the
                                 // unused space at the right of visible columns is hovered.
...
extern void ImGuiTestEngineHook_Log(ImGuiContext *ctx, const char *fmt, ...);
        ...
        #define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS) \
            if (g.TestEngineHookItems)                           \
            ImGuiTestEngineHook_ItemInfo(&g, _ID, _LABEL, _FLAGS) // Register item label and status
                                                                  // flags (optional)
        #define IMGUI_TEST_ENGINE_LOG(_FMT, ...) \
            if (g.TestEngineHookItems)           \
            ImGuiTestEngineHook_Log(&g,          \
                                    _FMT,        \
