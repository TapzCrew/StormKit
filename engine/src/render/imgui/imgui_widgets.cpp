...
void ImGui::Bullet() {
    ...
    const float line_height =
        ImMax(ImMin(window->DC.CurrLineSize.y, g.FontSize + g.Style.FramePadding.y * 2),
              g.FontSize);
...
TYPE ImGui::RoundScalarWithFormatT(const char *format, ImGuiDataType data_type, TYPE v) {
    ...
    const char *p = v_str;
    while (*p == ' ') p++;
    if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) v = (TYPE)ImAtof(p);
    else
...
bool ImGui::DragBehavior(ImGuiID id,
    ...
    ImGuiContext &g = *GImGui;
    if (g.ActiveId == id) {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse && !g.IO.MouseDown[0]) ClearActiveID();
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type,
    ...
    if (is_logarithmic) {
        ...
        if ((v_min == 0.0f) && (v_max < 0.0f)) v_min_fudged = -logarithmic_zero_epsilon;
        ...
        else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
        {
            ...
            if (v == 0.0f) result = zero_point_center; // Special case for exactly zero
            else if (v < 0.0f)
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar *text_begin,
    ...
    if (out_offset)
        *out_offset =
            ImVec2(line_width, text_size.y + line_height); // offset allow for the possibility of
...
namespace ImStb {
    ...
    static void
        STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, STB_TEXTEDIT_STRING *obj, int line_start_idx) {
...
bool ImGui::ColorPicker4(const char *label,
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        float cos_hue_angle = ImCos(H * 2.0f * IM_PI);
        float sin_hue_angle = ImSin(H * 2.0f * IM_PI);
        ImVec2 hue_cursor_pos(wheel_center.x +
...
bool ImGui::ColorButton(const char *desc_id,
    ...
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
        if (g.Style.FrameBorderSize > 0.0f) RenderFrameBorder(bb.Min, bb.Max, rounding);
        else
            window->DrawList
                ->AddRect(bb.Min,
                          bb.Max,
...
bool ImGui::BeginMainMenuBar() {
    ...
    if (menu_bar_window == NULL || menu_bar_window->BeginCount == 0) {
        // Set window position
        // We don't attempt to calculate our height ahead, as it depends on the per-viewport font
        // size. However menu-bar will affect the minimum window size so we'll get the right height.
        ImVec2 menu_bar_pos = viewport->Pos + viewport->CurrWorkOffsetMin;
        ImVec2 menu_bar_size =
            ImVec2(viewport->Size.x - viewport->CurrWorkOffsetMin.x + viewport->CurrWorkOffsetMax.x,
                   1.0f);
    ...
    PushStyleVar(ImGuiStyleVar_WindowMinSize,
                 ImVec2(0, 0)); // Lift normal size constraint, however the presence of a menu-bar
                                // will give us the minimum height we want.
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float min_w   = window->DC.MenuColumns.DeclColumns(label_size.x,
                                                         0.0f,
                                                         IM_FLOOR(g.FontSize *
                                                                  1.20f)); // Feedback to next frame
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float min_w =
            window->DC.MenuColumns.DeclColumns(label_size.x,
                                               shortcut_w,
                                               IM_FLOOR(g.FontSize *
                                                        1.20f)); // Feedback for next frame
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
...
namespace ImGui {
    ...
    static void
        TabBarScrollToTab(ImGuiTabBar *tab_bar, ImGuiID tab_id, ImGuiTabBarSection *sections);
...
static void
    ...
    if (tab == NULL) return;
    if (tab->Flags & (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)) return;
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list,
    ...
    if (flags & ImGuiTabItemFlags_UnsavedDocument) {
        ...
        ImVec2 unsaved_marker_pos(ImMin(bb.Min.x + frame_padding.x + label_size.x + 2,
