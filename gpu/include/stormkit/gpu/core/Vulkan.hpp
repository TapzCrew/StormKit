...
namespace stormkit::gpu {
    ...
    constexpr auto DEVICE_EXTENSIONS = std::array {
...
#else
    constexpr auto VALIDATION_LAYERS   = std::array<core::CZString, 0> {};
#endif
...
#if STORMKIT_WSI_ENABLED
    constexpr auto INSTANCE_EXTENSIONS = std::array {
...
#else
    constexpr auto INSTANCE_EXTENSIONS = std::array<std::string_view, 0> {};
#endif

#if defined(STORMKIT_RENDER_SAFE_VULKAN) || defined(STORMKIT_ENABLE_VALIDATION_LAYERS)
    constexpr auto ENABLE_VALIDATION = true;
#else
    constexpr auto ENABLE_VALIDATION   = false;
#endif
