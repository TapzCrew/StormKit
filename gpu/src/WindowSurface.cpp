...
module stormkit.Gpu:WindowSurface;
...
namespace stormkit::gpu {
    NAMED_LOGGER(window_surface_logger, "stormkit.Gpu:core.WindowSurface")
    ...
    WindowSurface::WindowSurface(const wsi::Window& window,
        ...
        #if STORMKIT_WSI_BUILD_WAYLAND and STORMKIT_WSI_BUILD_X11
    ...
    #endif
#else
        window_surface_logger.elog("WSI disabled in this build");
#endif
    };
...
#if STORMKIT_WSI_ENABLED
        m_device       = &device;
        m_raster_queue = &device.rasterQueue();
    ...
    auto WindowSurface::acquireNextFrame() -> Expected<Frame> {
        ...
        if (const auto result = in_flight.wait(std::chrono::milliseconds { 1000 });
            ...
            window_surface_logger.elog("Failed to acquire frame {} fence, reason: {}",
                                       m_current_frame,
                                       to_string(result));
    ...
    auto WindowSurface::present(const gpu::WindowSurface::Frame& frame) -> void {
        ...
        auto result = vk.vkQueuePresentKHR(m_device->rasterQueue(), &present_info);
        if (result == VK_ERROR_OUT_OF_DATE_KHR) [[unlikely]] {
            wlog("Out of date Swapchain, need to recreate");
            m_need_recreate = true;
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto WindowSurface::isPhysicalDeviceSupported(const PhysicalDevice& device,
                                                  core::Int32 family_index) const noexcept
        -> bool override {
        auto support = VkBool32 { VK_FALSE };

        CHECK_VK_ERROR(
            vkGetPhysicalDeviceSurfaceSupportKHR(device, family_index, vkSurface(), &support))
    ...
    auto WindowSurface::createSwapchain() -> void {
        ...
        for (const auto& vk_image : m_vk_images) {
            ...
            m_device->setObjectName(image, std::format("StormKit:SwapchainImage ({})", i++));
        }

        m_image_count = core::as<core::UInt32>(std::size(m_images));

        auto transition_commandbuffer = m_raster_queue->createCommandBuffer();

        transition_commandbuffer.begin(true);
        for (auto& image : m_images)
            transition_commandbuffer.transitionImageLayout(image,
                                                           ImageLayout::Undefined,
                                                           ImageLayout::Present_Src);
        transition_commandbuffer.end();

        auto fence = m_device->createFence();

        transition_commandbuffer.submit({}, {}, &fence);
    ...
    auto WindowSurface::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) noexcept
        ...
        if (capabilities.currentExtent.width != int_max and
